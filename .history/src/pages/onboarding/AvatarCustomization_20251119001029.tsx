...
const tabs: { id: TabId; label: string; options: AvatarOption[] }[] = [
  ...
  { id: 'hair', label: 'Hair', options: hairOptions },
  { id: 'features', label: 'Features', options: featureOptions },
  { id: 'shoes', label: 'Shoes', options: shoeOptions },
];
...
export default function AvatarCustomization() {
  ...
  const [selections, setSelections] = useState<Record<TabId, AvatarOption>>({
    ...
    <div className="avatar-customization">
      ...
      <div className="avatar-customization__options-grid">
        {activeTabConfig.options.map((option) => {
          const isSelected = selections[activeTabConfig.id].id === option.id;
          return (
            <button
              key={option.id}
              type="button"
