...
async function fetchColoredSymbol(
  symbolId: string,
  bodyColor: string
): Promise<string | null> {
  try {
...
export function AvatarCustomizer({
    ...
    switch (activeTab) {
      case "body":
        return [
          ...
          ...avatarOptions.bodyColors.map((color) => ({
            id: color,
            label: color,
          })),
        ];
      case "expression":
        const expressions =
          avatarOptions.expressions[
            selectedBody as keyof typeof avatarOptions.expressions
          ] || [];
      ...
      case "hair":
        const hairOptions = config.headwear
          ? avatarOptions.hair.filter(
              (id) => id === "hair-1" || id === "hair-7"
            )
          : avatarOptions.hair;
        const facialOptions =
      ...
      case "accessories":
        ...
        return [
          ...
          ...avatarOptions.accessories.map((id) => ({
            id,
            label: id,
            category: "accessories" as const,
          })),
  ...
  const handleOptionSelect = (
    ...
    if (optionId.startsWith("none-")) {
      const noneCategory = optionId.split("-")[1] as
        | "hair"
        | "facial"
        | "eyewear"
        | "headwear"
      ...
      if (activeTab === "hair") {
        ...
        if (noneCategory === "eyewear") {
          ...
          if (
            config.eyewear === "orange-mask" ||
            config.eyewear === "orange-mask-2"
          ) {
            updateConfig("eyewear", undefined);
          }
  ...
  const renderOptionPreview = (
    ...
    if (activeTab === "body") {
      ...
      if (category === "eyewear") {
        ...
        className =
          "avatar-option__preview avatar-option__preview--accessories";
      } else if (category === "facial") {
