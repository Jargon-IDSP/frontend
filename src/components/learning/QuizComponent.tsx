...
Remember: Be supportive, keep it brief, and explain like you're talking to a friend who's learning English and trades at the same time.`;
  ...
  return (
    <div>
      <button className="back-to-quiz-button" onClick={onBack} aria-label="Back to Quizzes">
        <img src={goBackIcon} alt="Back" />
      </button>

      <div>
        <span className="quzi-title">Quiz {quizNumber}</span>
        <span className="quzi-number">
          Question {currentQuestionIndex + 1} of {questions.length}
        </span>
      </div>
        ...
        <div className="quiz-choices-section">
          {currentQuestion.choices.map((choice) => {
            ...
            return (
              <button className={`quiz-choices-button ${[
              ...
              >
                ...
                {showFeedback && (
                  <div className="quiz-feedback-section">
                    {isSelected && isCorrect && (
                      <span className="quiz-correct">
                        ✓
                      </span>
                    )}
                    {isSelected && !isCorrect && (
                      <span className="quiz-incorrect">
                        ✗
                      </span>
                    )}
                    {!isSelected && isCorrect && (
      ...
      <button className="quiz-chat-button"
            onClick={() => setShowChatModal(true)}
          >
